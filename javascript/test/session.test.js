...
function create_session() {
    ...
    const session = alice.create_outbound_session(bob.curve25519_key, one_time_key);
...
describe('Olm Session', function () {
    ...
    it('should let us encrypt and decrypt messages', function () {
        ...
        const message = session.encrypt(encode(plaintext));
        expect(message.message_type).toBe(0);

        let { plaintext: decrypted, session: bob_session } = bob.create_inbound_session(alice.curve25519_key, message);

        expect(decode(decrypted)).toEqual(plaintext);
    ...
    it('should throw an exception if the message is not valid', function () {
        const [_alice, _bob, session] = create_session();
        const message = new OlmMessage(0, new Uint8Array([0]));
    ...
    it('should let us encrypt and decrypt multiple messages', function () {
        ...
        let message = session.encrypt(encode(plaintext));
        expect(message.message_type).toBe(0);
        ...
        message = bob_session.encrypt(encode(plaintext));
    ...
    it('should tell us if a pre-key message matches a session', function () {
        ...
        let { plaintext: _decrypted, session: bob_session } = bob.create_inbound_session(alice.curve25519_key, message);
    ...
    it('should throw an exception if the one-time key is invalid', function () {
        ...
        const message = new OlmMessage(0, new Uint8Array([0]));
        expect(() => bob.create_inbound_session(alice.curve25519_key, message)).toThrow();
    });
    ...
    it("should tell us if a pre-key message doesn't match a session", function () {
        ...
        message = new OlmMessage(0, new Uint8Array([0]));
        expect(bob_session.session_matches(message)).toBe(false);
    });
});
